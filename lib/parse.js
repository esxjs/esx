...
function findNearestTag (tree, tag) {
  var top = tree.length
  while (top-- > 0) {
    const el = tree[top]
    if (el[0] === tag) return el
  }
  return null
}
        ...
        case (ch === '>' && ctx !== DQ && ctx !== SQ):
          ...
          if (meta.isComponent === false) {
            ...
            if (r === '/option') {
              ...
              if (optionValue) field = field.slice(0, meta.openTagStart[1]) + 
                field.slice(meta.openTagStart[1], meta.openTagStart[1] + r.length) + 
                `\${this.selected(${JSON.stringify(optionValue)})}` + 
                field.slice(meta.openTagStart[1] + r.length)
            }
            if (r === '/select') {
