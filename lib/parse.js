...
function addProp (tree, name) {
  ...
  const props = node[1]
  const meta = node[3]
  if (name === 'dangerouslySetInnerHTML' && VOID_ELEMENTS.has(tag)) {
...
function valuelessAttr (tree, field, key, suffix = '') {
  ...
  if (debug.enabled) debug(`the attribute "${key}" has no value and is not a boolean attribute. This attribute will not be rendered.`)
      ...
      switch (true) {
        ...
        case (ch === '>' && ctx !== DQ && ctx !== SQ):
          ...
          if (meta.isComponent === false) {
            ...
            if (meta.childrenAttribute && meta.closed) {
              const hasNoChildren = field[field.length - r.length - 2] === '>'
              if (hasNoChildren) {
                field = field.slice(0, field.length - r.length - 1) + escapeHtml(props.children) + field.slice(-r.length-1)
              }
            }
          }
          if (!meta.openTagEnd) {
        ...
        case (ctx === VAL && /\s/.test(ch)):
          ...
          ctx = ATTR
          tokens(ATTR)
          if (tree[tree.length - 1][3].isComponent === false && lastKey === 'children') {
